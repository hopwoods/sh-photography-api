...
namespace Stuart_Hopwood_Photography_API.Helpers
{
   public class GoogleOAuthHelper : IOAuthHelper
   {
      ...
      public async Task<bool> RefreshToken(UserCredential credential)
      {
         ...
         using (var response = await _client.PostAsync(request.RequestUri, request.Content))
         {
            ...
            var responseObject = JsonConvert.DeserializeObject<GoogleAccessToken>(responseJson);

            var newToken = responseObject.Access_Token;
